...
max_gen_len = st.sidebar.slider(
    "📝 Max Generation Length", min_value=100, max_value=2048, value=1024, step=50
)
temperature = st.sidebar.slider(
    "🔥 Temperature (Creativity)", min_value=0.0, max_value=1.0, value=0.1, step=0.05
)
top_p = st.sidebar.slider(
    "🎯 Top-P Sampling (Diversity)", min_value=0.0, max_value=1.0, value=0.9, step=0.05
)
...
if prompt := st.chat_input("💬 Ask me anything!"):
    ...
    with st.spinner("🤖 Generating response... Please wait."):
        ...
        # Fix: Ensure response doesn't get cut off
